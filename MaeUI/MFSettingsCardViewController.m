...
-(id)init {
  ...
  float buttonSpacing = 32; // yes. i know its hardcoded. i dont care... for now.
...
-(void)setupAirplaneToggleWithSpacing:(float)buttonSpacing size:(float)buttonSize {
  ...
  self.airplaneButton = [MFRoundButton new];
  self.airplaneButton.translatesAutoresizingMaskIntoConstraints = NO;
  self.airplaneButton.layer.cornerRadius = buttonSize / 2;
  self.airplaneButton.backgroundColor = airState ? [UIColor systemOrangeColor] : [UIColor colorWithWhite:(2/3) alpha:0.1];
...
-(void)setupWifiToggleWithSpacing:(float)buttonSpacing size:(float)buttonSize {
  ...
  self.wifiButton = [MFRoundButton new];
  self.wifiButton.translatesAutoresizingMaskIntoConstraints = NO;
  self.wifiButton.layer.cornerRadius = buttonSize / 2;
  self.wifiButton.backgroundColor = wfState ? [UIColor systemBlueColor] : [UIColor colorWithWhite:(2/3) alpha:0.1];
...
-(void)setupBluetoothToggleWithSpacing:(float)buttonSpacing size:(float)buttonSize {
  ...
  self.bluetoothButton = [MFRoundButton new];
  self.bluetoothButton.translatesAutoresizingMaskIntoConstraints = NO;
  self.bluetoothButton.layer.cornerRadius = buttonSize / 2;
  self.bluetoothButton.backgroundColor = blueState ? [UIColor systemBlueColor] : [UIColor colorWithWhite:(2/3) alpha:0.1];
...
-(void)toggleAirplaneMode:(MFRoundButton*)sender {
  ...
  if (![airToggle currentState]) {
    ...
    [UIView animateWithDuration:0.2 animations:^{
      self.airplaneButton.backgroundColor = [UIColor colorWithWhite:(2/3) alpha:0.1];
    }];
...
-(void)wifiPowerDidChange {
  dispatch_async(dispatch_get_main_queue(), ^{
    [UIView animateWithDuration:0.2 animations:^{
      self.wifiButton.backgroundColor = ([wifiToggle currentState]) ? [UIColor systemBlueColor] : [UIColor colorWithWhite:(2/3) alpha:0.1];
    }];
...
-(void)bluetoothPowerDidChange {
  [UIView animateWithDuration:0.2 animations:^{
    self.bluetoothButton.backgroundColor = ([bluetoothToggle currentState]) ? [UIColor systemBlueColor] : [UIColor colorWithWhite:(2/3) alpha:0.1];
  }];
